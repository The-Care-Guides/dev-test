...
// Reseed before the suite so every run starts from the same fixed data, the way
// the Java implementation gets a fresh in-memory H2 on each run.
beforeAll(async () => {
  try {
    await prisma.performance.count();
  } catch {
    throw new Error('Run "npm run db:setup" before running tests');
  }
  await seed();
});
